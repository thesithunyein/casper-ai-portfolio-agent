...
export const AIAnalysisComponent = ({ analysis }: AIAnalysisProps) => {
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      ...
      {/* On-chain Record — primary judge proof */}
      {analysis.onchain && (
        <div className="bg-white dark:bg-ink-900 border border-emerald-200 dark:border-emerald-500/20 rounded-xl p-5 shadow-stripe-sm">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[10px] font-mono text-emerald-600 bg-emerald-500/5 border border-emerald-500/20 px-1.5 py-0.5 rounded">
              PROOF
            </span>
          </div>
          <div className="space-y-2">
            <div className="p-3 bg-ink-50 dark:bg-ink-800/50 border border-black/[0.06] dark:border-white/[0.06] rounded-lg">
              <p className="text-[10px] font-mono text-ink-400 dark:text-ink-500 uppercase mb-1 tracking-wider">store_analysis Tx</p>
              <a
      ...
      {/* x402 Micropayment */}
      {(analysis.x402Status || analysis.x402Payment || analysis.x402Error) && (
        <div className="bg-white dark:bg-ink-900 border border-sky-200 dark:border-sky-500/20 rounded-xl p-5 shadow-stripe-sm">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded border ${
              ...
              {analysis.x402Status === 'settled' ? 'SETTLED ON-CHAIN' : analysis.x402Status === 'verified' ? 'VERIFIED' : analysis.x402Error ? 'SETTLE FAILED' : 'OPTIONAL'}
            </span>
          </div>
          {analysis.x402Payment?.explorerUrl ? (
            ...
            <p className="text-sm text-ink-500 dark:text-ink-400">
              {analysis.x402Error
                ? `Agent-wallet settle failed: ${analysis.x402Error}. On-chain proof is still available via store_analysis above when present.`
                : analysis.onchain
                  ? 'x402 facilitator settle not completed this run. Primary on-chain proof is the store_analysis transaction above.'
                  : 'Payment header structurally verified. Agent-wallet settle will post a native transfer when the network accepts it.'}
            </p>
          )}
        </div>
      )}
      ...
      {analysis.autonomousAction && (
        <div className="bg-white dark:bg-ink-900 border border-violet-200 dark:border-violet-500/20 rounded-xl p-5 shadow-stripe-sm">
          ...
          <div className="space-y-2">
            ...
            <div className="p-3 bg-ink-50 dark:bg-ink-800/50 border border-black/[0.06] dark:border-white/[0.06] rounded-lg">
              <p className="text-[10px] font-mono text-ink-400 dark:text-ink-500 uppercase mb-1 tracking-wider">Action</p>
              <p className="text-xs font-mono text-ink-600 dark:text-ink-300">Native CSPR transfer</p>
            </div>
          </div>
        </div>
      )}
