...
export const PortfolioDisplay = ({ portfolio }: PortfolioDisplayProps) => {
  return (
    <div className="bg-surface border border-border p-4 animate-fade-in">
      ...
      <div className="space-y-2">
        {portfolio.assets.map((asset) => (
          <div key={asset.denom} className="p-3 bg-surface-alt border border-border hover:border-border-strong transition-colors">
            <div className="flex justify-between items-center mb-2">
              <div className="flex items-center gap-2">
                <span className="w-6 h-6 bg-black text-white flex items-center justify-center text-[10px] font-bold">
                  {asset.symbol[0]}
                </span>
                <div>
                  <p className="text-sm font-medium text-black">{asset.symbol}</p>
            ...
            <div className="w-full bg-surface border border-border h-1.5">
              <div className="bg-black h-full" style={{ width: `${asset.percentage}%` }} />
            </div>
          </div>
        ))}
      </div>
